...
@pcc_set
class WenhankPeterantLink(Link):
    USERAGENTSTRING = "WenhankPeterant"
...
@subset(WenhankPeterantLink)
class WenhankPeterantUnprocessedLink(object):
    @staticmethod
    def __predicate__(l):
...
@impure
@subset(WenhankPeterantUnprocessedLink)
class OneWenhankPeterantUnProcessedLink(WenhankPeterantLink):
    __limit__ = 1
    @staticmethod
    def __predicate__(l):
        return not (l.download_complete or l.error_reason)

@projection(WenhankPeterantLink, WenhankPeterantLink.url, WenhankPeterantLink.download_complete)
class WenhankPeterantProjectionLink(object):
    pass

@trigger(OneWenhankPeterantUnProcessedLink, TriggerTime.after, TriggerAction.read)
...
@trigger(WenhankPeterantLink, TriggerTime.after, TriggerAction.update)
