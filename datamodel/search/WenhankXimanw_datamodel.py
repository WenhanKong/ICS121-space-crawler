...
@pcc_set
class WenhankXimanwLink(Link):
    USERAGENTSTRING = "WenhankXimanw"
...
@subset(WenhankXimanwLink)
class WenhankXimanwUnprocessedLink(object):
    @staticmethod
    def __predicate__(l):
...
@impure
@subset(WenhankXimanwUnprocessedLink)
class OneWenhankXimanwUnProcessedLink(WenhankXimanwLink):
    __limit__ = 1
    @staticmethod
    def __predicate__(l):
        return not (l.download_complete or l.error_reason)

@projection(WenhankXimanwLink, WenhankXimanwLink.url, WenhankXimanwLink.download_complete)
class WenhankXimanwProjectionLink(object):
    pass

@trigger(OneWenhankXimanwUnProcessedLink, TriggerTime.after, TriggerAction.read)
...
@trigger(WenhankXimanwLink, TriggerTime.after, TriggerAction.update)
